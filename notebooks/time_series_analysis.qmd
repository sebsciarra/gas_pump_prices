...
df_inflation = read_csv('data/raw/1810000501_databaseLoadingData.csv')
df_gas_inf = df_inflation |> filter(`Products and product groups` == 'Gasoline')

df_gasr$year = as.integer(format(df_gasr$Date, "%Y"))

# compute dollars in terms of 2025 dollars
# 1) Compute CPIs centered on 2025 
value_24 = df_gas_inf |> filter(REF_DATE == 2024) |> pull(VALUE)
  = df_gas_inf$VALUE/value_24

# merge in CPI value based on year
cols = c('REF_DATE', 'VALUE', 'cpi_2024')
df_gasr <- left_join(df_gasr, df_gas_inf[ ,cols], by = c("year" = "REF_DATE"))
