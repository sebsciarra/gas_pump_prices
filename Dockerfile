...
# and `pip install gas_pump_prices` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "gas_pump_prices", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n gas_pump_prices -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "gas_pump_prices", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "gas_pump_prices", "YOUR_CONSOLE_SCRIPT"]
